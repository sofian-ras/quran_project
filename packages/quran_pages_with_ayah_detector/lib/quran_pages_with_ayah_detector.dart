...
// ── Inline replacements for removed quran package ────────────────────────────
...
const List<int> _juzStartPages = [
  1,22,42,62,82,102,121,142,162,182,
  201,221,242,262,282,301,322,342,362,382,
  401,421,442,462,482,502,522,542,562,582,
];

String _getSurahNameArabic(int surah) {
  if (surah < 1 || surah > 114) return '';
  return _surahArabicNames[surah - 1];
}

int _getJuzNumber(int surah, int ayah) {
  final page = suraAyahToPage[surah]?[ayah] ?? 1;
  for (int j = _juzStartPages.length - 1; j >= 0; j--) {
    if (_juzStartPages[j] <= page) return j + 1;
  }
  return 1;
}

Map<int, List<int>> _getSurahsInJuz(int juzNum) {
  final startPage = _juzStartPages[juzNum - 1];
  final endPage = juzNum < 30 ? _juzStartPages[juzNum] - 1 : 604;
  final result = <int, List<int>>{};
  for (int s = 1; s <= 114; s++) {
    final pageStart = suraAyahToPage[s]?[1] ?? 1;
    if (pageStart >= startPage && pageStart <= endPage) {
      result[s] = List.generate(suraNumberOfAyahs[s] ?? 7, (i) => i + 1);
    }
  }
  return result;
}
...
class _QuranPageViewState extends State<QuranPageView>
  ...
  int _getCurrentSurahForPage(int page) {
    ...
    for (int s = 1; s <= 114; s++) {
      ...
      final lastAyah = suraNumberOfAyahs[s] ?? 7;
  ...
  void _performSearch(String query) {
    ...
    for (int i = 1; i <= 114; i++) {
      final suraName = _getSurahNameArabic(i);
                              ...
                              for (int juzIndex = 0; juzIndex < 30; juzIndex++)
                                Builder(
                                  builder: (context) {
                                    ...
                                    final juzData = _getSurahsInJuz(juzNum);
                                    ...
                                    final filteredSuras = juzSuras.where((s) {
                                      final startJuz = _getJuzNumber(s, 1);
                                      if (startJuz != juzNum) return false;

                                      if (sheetSearchQuery.isNotEmpty) {
                                        final name = _getSurahNameArabic(s);
